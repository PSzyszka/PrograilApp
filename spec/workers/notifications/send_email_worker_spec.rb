...
RSpec.describe Notifications::SendEmailWorker, type: :worker do
  ...
  let(:subscription)           { create(:subscription, user: user_restaurant.user) }
  let(:confirmed_subscription) { create(:subscription, :email_confirmed, user: user_restaurant.user) }
  ...
  describe '#perform' do
    context 'when a subscription is confirmed' do
      context 'when post id is present' do
        ...
        it 'executes SendUserNotificationMailer' do
          ...
          expect(SendUserNotificationMailer).
            to receive(:with).
            with(subscription: confirmed_subscription, post: post).
            and_return(mailer)
    ...
    context 'when no subscription is confirmed' do
      it 'returns nil' do
        subscription

        expect(scheduled_valid_job).to be_nil
      end
    end
  end
end
